...
export class Schlepner extends base_class(ImageSplatter).and_interfaces(
  ZESpatialPhagy
) {
  //
  ...
  ZESpatialHarden(e) {
    if (this.grab_prov != null) return 0;
    //let hit = this.SelfWhack (e);

    const cm = this.CurrentCumuMats();
    if (cm == null) return 0;
    ...
    const hit = Geom.RayRectIntersection(e.loc, e.aim, cnt, o, u, w, h);

    if (hit == null) return 0;

    this.grab_prov = e.Provenance();
    const offset = hit.Sub(this.CurLoc());
  ...
  ZESpatialMove(e) {
    ...
    if (this.grab_prov == null || this.grab_prov != prov) return 0;

    const mah = PlatonicMaes.ClosestAmong(
      this.bang.Maeses(),
      e.loc,
      e.aim,
      true
    ...
    const [maes, hit] = mah;
    if (maes != this.cur_maes) this.AlignToMaes((this.cur_maes = maes));
    const cm = this.CurrentCumuMats();
    if (cm != null) {
      const o = cm.nmat.TransformVect(Vect.xaxis);
      const u = cm.nmat.TransformVect(Vect.yaxis);
      hit.SubAcc(o.Sca(this.grab_off.x)).SubAcc(u.Sca(this.grab_off.y));
    }
    this.SetLoc(hit);
    return 0;
  }
