...
export class PolygonPile extends Alignifer {
  ...
  SetFillColor(fc) {
    if (fc === this.fill_iro) return this;
    if (this.fill_iro == null || fc == null) this.fill_iro = fc;
    else this.fill_iro.Set(fc);
    return this;
  }
  ...
  SetStrokeColor(sc) {
    if (sc === this.strk_iro) return this;
    if (this.strk_iro == null || sc == null) this.strk_iro = sc;
    else this.strk_iro.Set(sc);
    return this;
  }
  ...
  NthPoly(ind) {
    if (ind < 0 || ind >= this.poly_arr.length) return null;
  ...
  DrawSelf(ratch, cm, adjc, bonus) {
    ...
    const polys = this.CanvasProjectVertexArrays(
      cm,
      bonus[2],
      bonus[0],
      this.poly_arr
