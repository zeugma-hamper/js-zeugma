...
export class IronLung extends Zeubject {
  ...
  Inhale(ratch, thyme) {
    if (this.expirees.length > 0) {
      for (const z of this.expirees)
        if (this.IndexOfBreathee(z) >= 0) this._RemoveNthBreathee(z);
      this.expirees = new Array();
    }
    if (this.aspirants.length > 0) {
      for (const z of this.aspirants) this._AppendBreathee(z);
      this.aspirants = new Array();
    }

    let z = this.NumBreathees();
    const cnt = this.NumBreathees();
    let ded_cnt = 0;
    for (let q = 0; q < cnt; ++q)
      if ((z = this.NthBreathee(q)) == null) ++ded_cnt;
      else z.Inhale(ratch, thyme);
  ...
  AppendBreathee(z, record_self_in_breathee) {
    ...
    if (record_self_in_breathee == undefined) record_self_in_breathee = true;
    if (record_self_in_breathee) z.SetBreather(this);
    return true;
  }
  ...
  _RemoveBreathee(z) {
    ...
    if (urn && z.Breather() != null) z.SetBreather(null);
    return urn;
  }
  RemoveBreathee(z) {
    ...
    if ((ind = this.aspirants.indexOf(z)) >= 0) {
      this.aspirants.splice(ind, 1);
      urn = true;
    }
    if (this.expirees.indexOf(z) < 0) {
      this.expirees.push(z);
      urn = true;
    }
    return urn;
  }
