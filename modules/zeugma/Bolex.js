...
export class Bolex extends SpaceThing {
  ...
  ViewCOI() {
    return this.z_view_loc
      .Val()
      .Add(this.z_view_aim.Val().Mul(this.z_view_dist.Val()));
  }
  ...
  ViewPlaneWidth() {
    if (this.ProjectionTypeIsOrthographic()) return this.ViewOrthoWid();
    return this.ViewDist() * 2.0 * Math.tan(0.5 * this.ViewHorizAngle());
  }
  ViewPlaneHeight() {
    if (this.ProjectionTypeIsOrthographic()) return this.ViewOrthoHei();
  ...
  SetViewLoc(loc) {
    this.z_view_loc = Zoft.NewWith(loc);
    return this;
  }

  SetViewAim(aim) {
    this.z_view_aim = Zoft.NewWith(aim);
    return this;
  }
  ...
  SetViewUp(up) {
    this.z_view_upp = Zoft.NewWith(up);
    return this;
  }

  SetViewDist(dst) {
    this.z_view_dist = Zoft.NewWith(dst);
    return this;
  }

  SetViewHorizAngleD(hdeg) {
    this.z_view_hrz_ang_d = Zoft.NewWith(hdeg);
    return this;
  }

  SetViewVertAngleD(vdeg) {
    this.z_view_vrt_ang_d = Zoft.NewWith(vdeg);
    return this;
  }
  ...
  SetViewOrthoWid(owid) {
    this.z_view_ortho_wid = Zoft.NewWith(owid);
    return this;
  }

  SetViewOrthoHei(ohei) {
    this.z_view_ortho_hei = Zoft.NewWith(ohei);
    return this;
  }
  ...
  SetViewPlaneOffset(
    ...
    this.z_view_pln_off = Zoft.NewWith(off);
    return this;
  }
  ...
  SetProjectionType(pt) {
    if (!Object.values(Bolex.ProjType).includes(pt))
      throw new Error(
        "hey now: from Bolex.SetProjectionType(), " +
          pt +
  ...
  LoadViewMatrixInto(vmat) {
    ...
    const cootr = new Matrix44().LoadBackwardCoordTransformPreNormed(
      ovr,
      uup,
      aim.Neg()
    );
  ...
  LoadProjectionMatrixInto(pmat) {
    ...
    pmat.LoadShear(0.0, poff.X() / vdst, 0.0, poff.Y() / vdst, 0.0, 0.0);
