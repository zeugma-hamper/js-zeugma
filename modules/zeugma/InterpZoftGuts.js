...
export class InterpZoftGuts extends ZoftGuts {
  ...
  Pause() {
    this.recent_time = -Math.abs(this.recent_time);
    return this;
  }
  ...
  Finish() {
    this.recent_time = this.interp_time;
    return this;
  }
  ...
  Reverse() {
    ...
    this.z_pnt_b = tmp;
    if (this.replete) this.Commence();
    else this.recent_time = this.interp_time - this.recent_time;
    return this;
  }
  ...
  SetGoal(goal_zoft_or_val) {
    this.ProceedTo(goal_zoft_or_val);
    this.Commence();
    return this;
  }
  ...
  Inhale(ratch, thyme) {
    ...
    if (t > 1.0) t = 1.0;
    const v = Zoft.OffalAdd(
      Zoft.OffalSca(this.z_pnt_a.Val(), 1.0 - t),
