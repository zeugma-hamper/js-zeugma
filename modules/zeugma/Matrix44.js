...
export class Matrix44 {
  ...
  Neg() {
    ...
    for (let q = 15; q >= 0; --q) arr[q] = -this.m[q];
    return new Matrix44()._SetGuts(arr);
  }

  NegSelf() {
    for (let q = 15; q >= 0; --q) this.m[q] = -this.m[q];
    return this;
  }
  ...
  LoadRotationPreNormed(axs, ang) {
    if (axs.IsZero()) return this.LoadIdent();
  ...
  LoadShearAbout(x_by_y, x_by_z, y_by_x, y_by_z, z_by_x, z_by_y, cnt) {
    this.LoadTranslation(cnt.Neg());
    const emm = new Matrix44().LoadShear(
      x_by_y,
      x_by_z,
      y_by_x,
      y_by_z,
  ...
  //
  LoadForwardCoordTransformPreNormed(
    o,
    u,
    n // Vects all, y'see.
  ) {
  ...
  LoadForwardCoordTransformPreNormedOverUp(
    o,
    u // both Vects, don't you know.
  ) {
    return this.LoadForwardCoordTransformPreNormed(o, u, o.Cross(u));
  }

  LoadForwardCoordTransform(
    o,
    u,
    n // still Vects, but not normalized. dig?
  ) {
    return this.LoadForwardCoordTransformPreNormed(
      o.Norm(),
      u.Norm(),
      n.Norm()
    );
  ...
  LoadForwardCoordTransformOverUp(
    o,
    u // them's, yep, Vects: normless.
  ) {
    return this.LoadForwardCoordTransformPreNormedOverUp(o.Norm(), u.Norm());
  }

  LoadBackwardCoordTransformPreNormed(
    o,
    u,
    n // as above.
  ) {
  ...
  LoadBackwardCoordTransformPreNormedOverUp(
    o,
    u // see the foreforegoing.
  ) {
    return this.LoadBackwardCoordTransformPreNormed(o, u, o.Cross(u));
  }

  LoadBackwardCoordTransform(
    o,
    u,
    n // oh yes. look upward.
  ) {
    return this.LoadBackwardCoordTransformPreNormed(
      o.Norm(),
      u.Norm(),
      n.Norm()
    );
  ...
  LoadBackwardCoordTransformOverUp(
    o,
    u // plenty more of same.
  ) {
    return this.LoadBackwardCoordTransformPreNormedOverUp(o.Norm(), u.Norm());
  }
  ...
  TransformVectArray(varr) {
    ...
    if (varr != null)
      for (const vec of varr) outarr.push(this.TransformVect(vec));
    return outarr;
  }

  TransformVectArrayInPlace(varr) {
    if (varr != null) for (const vec of varr) this.TransformVectInPlace(vec);
    return varr;
  }
