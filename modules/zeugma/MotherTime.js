...
export class MotherTime {
  ...
  CurTime() {
    if (this.chrono_freeze) return this.nowesque_time;
  ...
  DeltaTime() {
    if (this.chrono_freeze) return 0.0;
  ...
  SetTimePausedness(bool_tp) {
    if (this.chrono_freeze == bool_tp) return this.nowesque_time;
    const cur = this.CurTime();
    this.chrono_freeze = bool_tp;
    if (!bool_tp) this.prev_millisecs = this.constructor.MILLISECS_OF_NOW();
    return cur;
  }
  ...
  ToggleTimePausedness() {
    return this.SetTimePausedness(this.TimePausedness() ? false : true);
  }
