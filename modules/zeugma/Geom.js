...
export class Geom {
  ...
  static PointOntoPlaneProjection(
    pnt,
    pl_cnt,
    pl_nrm // Vects all...
  ) {
  ...
  static RayRectIntersection(
    ...
    const p = pee.Sub(cnt);
    let t = 2.0 * p.Dot(ovr);
    if (t < -wid || t > wid) return null;
    t = 2.0 * p.Dot(upp);
    if (t < -hei || t > hei) return null;
    return pee;
  }
  ...
  static RayAnnulusIntersectionPolar(frm, aim, cnt, e0, e1, r1, r2) {
    ...
    if (arr_sq < r1 * r1 || arr_sq > r2 * r2) return null;
    return new PolarHit(
      hit,
      Math.sqrt(arr_sq),
  ...
  static LineAnnulusIntersectionPolar(ln_pnt, ln_dir, cnt, e0, e1, r1, r2) {
    const hit = this.LinePlaneIntersection(ln_pnt, ln_dir, cnt, e0.Cross(e1));
    if (hit == null) return null;
    const h = hit.Sub(cnt);
    const arr_sq = h.AutoDot();
    if (arr_sq < r1 * r1 || arr_sq > r2 * r2) return null;
  ...
  static LineDiskIntersectionPolar(ln_pnt, ln_dir, cnt, e0, e1, r) {
    return this.LineAnnulusIntersectionPolar(
      ln_pnt,
      ln_dir,
      cnt,
      e0,
  ...
  static LineDiskIntersection(ln_pnt, ln_dir, cnt, e0, e1, r) {
    return this.LineAnnulusIntersection(ln_pnt, ln_dir, cnt, e0, e1, 0.0, r);
  }
  ...
  static _RaySphereIntersectionSolutions(frm, aim, cnt, rad) {
    ...
    if (unroo < 0.0) return emit;
    a = 0.5 / a;
    if (unroo == 0.0) emit[0] = -b * a;
    else {
  ...
  static RaySphereIntersections(frm, aim, cnt, rad) {
    const solns = this._RaySphereIntersectionSolutions(frm, aim, cnt, rad);
    if (solns[0] == null) return null;
    const emit = [null, null];
    emit[0] = frm.Add(aim.Sca(solns[0]));
    if (solns[1] != null) emit[1] = frm.Add(aim.Sca(solns[0]));
    return emit;
  }
