...
export class Loopervisor extends Zeubject {
  ...
  OnceMoreUntoTheBreath() {
    for (const ump of this.active_sumps) if (ump != null) ump.ProcessSump();

    for (const qued of this.active_aqueducts)
      if (qued != null) qued.DrainReservoir();

    this.FreshenTimestamps();

    for (const ung of this.active_lungs)
      if (ung != null) ung.Inhale(this.grand_ratchet, this.recentest_time);
  ...
  SpinGloriously() {
    if (this.is_looping == true) return this;
    ...
    const _SpinGlo = function () {
      ...
      if (selfesque.is_looping) setTimeout(_SpinGlo, dt > 0 ? dt * 1000 : 0);
    };
  ...
  Travail(ratch, thyme) {
    ...
    let ded_cnt = 0;
    for (let q = 0; q < cnt; ++q)
      if ((toi = this.NthToiler(q)) == null) ++ded_cnt;
      else toi.Travail(ratch, thyme);
