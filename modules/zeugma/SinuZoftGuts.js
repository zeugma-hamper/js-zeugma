...
export class SinuZoftGuts extends ZoftGuts {
  ...
  Inhale(ratch, thyme) {
    ...
    if (this.z_cent != null) this.z_cent.Inhale(ratch, thyme);
    if (this.z_phas != null) this.z_phas.Inhale(ratch, thyme);

    let v = thyme * this.z_freq.Val();
    v -= Math.trunc(v);
    v *= 2.0 * Math.PI;
    if (this.z_phas) v += this.z_phas.Val();
    v = Zoft.OffalSca(this.z_ampl.Val(), Math.sin(v));
    if (this.z_cent) v = Zoft.OffalAdd(v, this.z_cent.Val());
