...
export class PouchyThing extends base_class(Zeubject).and_interfaces(
  IContainMultitudes
) {
  //
  constructor() {
  ...
  AppendChild(z) {
    ...
    if (this.chirrens == null) this.chirrens = new Array();
    else if (this.chirrens.includes(z)) return false;
  ...
  RemoveChild(z) {
    if (this.chirrens == null || z == null) return false;
  ...
  IndexForChild(z) {
    if (this.chirrens == null || z == null) return -1;
    return this.chirrens.indexOf(z);
  }

  FindChild(
    nm // a string. it's the name, you dig?
  ) {
    if (this.chirrens == null) return null;
