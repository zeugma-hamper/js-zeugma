...
export class PlatonicMaes extends SpaceThing {
  ...
  NthLayer(ind) {
    if (ind < 0 || ind >= this.layers.length) return null;
  ...
  InsertLayer(z, ind) {
    if (z == null) return false;
    if (ind < 0) ind = 0;
    else if (ind > this.layers.length) ind = this.layers.length;
